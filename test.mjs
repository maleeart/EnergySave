...
const { emmChart, avgOf, MATRIX, coverage, HEADCOUNT } =
  vm.runInContext("({emmChart, avgOf, MATRIX, coverage, HEADCOUNT})", ctx);
...
assert.equal(c.pct, null, "ยังไม่ตั้งกำลังพล ต้องไม่คำนวณร้อยละ");
...
// เปิด Upstash REST จำลองบน localhost แล้วชี้ env มาที่นี่
// (mock ES module ไม่ได้ และวิธีนี้ได้ทดสอบ @upstash/redis ตัวจริงไปด้วย)
const { createServer } = await import("node:http");
const server = createServer((rq, rs) => {
  rs.setHeader("content-type", "application/json");
  rs.end(JSON.stringify({ result: people2.map(p => JSON.stringify(p)) }));
});
...
assert.equal(s1.getRow(2).getCell(12).value, 2.83, "คะแนนเฉลี่ยต้องคำนวณถูก");
...
assert.equal(s2.rowCount, 8, "หัวตาราง + 6 หัวข้อ + แถวภาพรวม");
assert.equal(s2.getRow(2).getCell(2).value, 3, "ข้อ 1: (4+2)/2 = 3");
assert.equal(s2.getRow(8).getCell(3).value, 3, "ภาพรวมต้องปัดเป็นระดับ 3");
